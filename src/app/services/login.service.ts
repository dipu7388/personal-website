...
export class LoginService {
  ...
  public login(user: UserModel): Promise<{
    ...
    return new Promise((resolve, reject) => {
      this.getSalt().then((salt) => {
        ...
        this.httpServices
          .post('loginUser', new Map().set('hiddenSalt', salt), user)
          .then(
            (response) => {
  ...
  getSalt(): Promise<string> {
    return new Promise<string>((resolve, reject) => {
      this.httpServices.get('getHiddenSalt', new Map()).then(
        (value) => {
          resolve(value.data);
        },
        (err) => {
  ...
  changePassword(newPassword: string): Promise<String> {
    ...
    return new Promise<string>((resolve, reject) => {
      this.httpServices
        .get('changepassurl', new Map().set('password', tempPassword))
        .then(
          (value) => {
            if (!value.error) {
              this.snackBar.open(value.msgList.join(' '), '', {
                duration: Constants.SANCKBAR_DURATION,
              });
